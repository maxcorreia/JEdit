...
/**
 * A class to represent the horizontal flipping of all pixels in an image.
...
public class HorizontalFlip implements Command {
  ...
  /**
   * Flips an ImageModel horizontally.
   *
  ...
  public static ImageModel execute(ImageModel model) {
    ...
    for (int row = 0; row < model.getHeight(); row++) {
      ...
      for (int col = 0; col < model.getWidth(); col++) {
        image.get(row).add(model.getColorAt(row, model.getWidth() - col - 1));
      }
    }
